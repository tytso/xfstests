	...
	int			fd)
{
	...
	if (want_attrfile && (fsx.fsx_xflags & FS_XFLAG_HASATTR))
		return 1;
	return 0;
}
	...
	int			fd)
{
	...
	memset(&bmaps[0], 0, sizeof(struct getbmapx));
	bmaps[0].bmv_length = ULLONG_MAX;
	bmaps[0].bmv_iflags = BMV_IF_PREALLOC | BMV_IF_NO_HOLES;
	bmaps[0].bmv_count = BMAP_NR;

	while ((ret = ioctl(fd, XFS_IOC_GETBMAPX, bmaps)) == 0) {
		struct getbmapx	*p = &bmaps[1];

		for (i = 0; i < bmaps[0].bmv_entries; i++, p++) {
