	...
	long			r)
{
	...
	if (!get_fname(FT_REGm, r, &fpath1, NULL, NULL, &v1)) {
		if (v1)
			printf("%d/%d: xchgrange read - no filename\n",
				procid, opno);
		goto out_fpath1;
	}
	...
	if (!get_fname(FT_REGm, random(), &fpath2, NULL, NULL, &v2)) {
		if (v2)
			printf("%d/%d: xchgrange write - no filename\n",
				procid, opno);
		goto out_fpath2;
	}
	...
	if (fd1 < 0) {
		if (v1)
			printf("%d/%d: xchgrange read - open %s failed %d\n",
	...
	if (fd2 < 0) {
		if (v2)
			printf("%d/%d: xchgrange write - open %s failed %d\n",
	...
	if (fstat64(fd1, &stat1) < 0) {
		if (v1)
			printf("%d/%d: xchgrange read - fstat64 %s failed %d\n",
	...
	if (fstat64(fd2, &stat2) < 0) {
		if (v2)
			printf("%d/%d: xchgrange write - fstat64 %s failed %d\n",
	...
	if (stat1.st_size < (stat1.st_blksize * 2) ||
		...
		if (v2)
			printf("%d/%d: xchgrange - files are too small\n",
				procid, opno);
		goto out_fd2;
	}
...
retry:
	...
	if (v1 || v2) {
		printf("%d/%d: xchgrange %s%s [%lld,%lld] -> %s%s [%lld,%lld]",
			procid, opno,
